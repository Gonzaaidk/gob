 /*
* GonzaaBot es una creación de Gonzaa idk
* GonzaaBot no tiene ningun fin de lucro
* Gonzaa idk se reserva todos los derechos de autor
* © 2021 Gonzaa idk, INC.
...
const { gonzatera } = require('./src/gonzatera')
...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Gonzaa idk\n' // Nombre
            + 'ORG:Gonzaa idk;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=12186560666:+1 218 656 0666\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
/******ARCHIVOS ANTILINK POR GONZAA******/
const antilink = JSON.parse(fs.readFileSync('./src/antilink.json'))

/******FIN DE ARCHIVOS ANTILINK POR GONZAA******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'GonzaaBot'; if (!author) author = '@gonzaa.idk';	
...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'Conectado by Gonzaa idk!')
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hoolaaaa @${num.split('@')[0]}\nComo estas?, espero que bien!!!! Bienvenido a *${mdata.subject}* el mejor grupo locura!\n\nUn gusto conocerte bebe😜\n\nOjito sigue las reglas del grupo si no para fuera, los admins te eliminan por gil😆\n\nPara utilizar el bot registrate con el comando ${prefix}daftar y tu nombre\n\nPara ver los demas comandos utiliza ${prefix}help\n\nOjito con el spam🤬\n\nby Gσηzαα ι∂к`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const time = moment.tz('America/Argentina/Catamarca').format('HH:mm:ss')
			const date = moment.tz('America/Argentina/Catamarca').format('DD/MM/YY')
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa GonzaaBot',
					...
					daftarB: `「¡REGISTRATEE!」\n\nPA' NO APARECES EN MI BASE DE DATOS🚫\n\nPara poder usarme escribe el siguente comando\n\nComando: ${prefix}daftar Nombre\nEjemplo: ${prefix}daftar Gσηzαα ι∂к`,
				}
			}
    			const apakah = ['Si','No']
                        const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
			const botNumber = client.user.jid
			const ownerNumber = ["12186560666@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '12186560666@s.whatsapp.net'
                        /******Entrada ApiKey******/
       ...
       /******ENTRADA FIN DE FUNCIONES******/
		...
		break
		case 'gonzatera':
		client.sendMessage(from, gonzatera(prefix, sender), text, {quoted: mek})
		break
		case 'owner':
		client.sendMessage(from, owner(prefix, sender), text, {quoted: mek})
		break
...
break				
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
	    case 'owner':
                case 'creator':
                client.sendMessage(from, {displayname: "Gonzaa idk", vcard: vcard}, MessageType.contact, { quoted: mek})
		client.sendMessage(from, 'Arriba está el número del creador del bot <Gσηzααвσт ву Gσηzαα ι∂к>\n\nNO SOY UN BOT LPM 🥸\n\nAhi puedes resolver tus preguntas y errores :)\n\nEste no es el numero del propietario del bot que estas usando, si no del creador de la base de datos del bot❗\n\nву Gσηzαα ι∂к',MessageType.text, { quoted: mek} )
	...
	case 'hidetag':
               ...
               break
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista De Los Pijudos Del Grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MENCIONANDO 〙✪══\n╠➥'+teks+'╚═〘 by Gσηzαα ι∂к 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
       ...
       /*case 'ownergrup':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `El mas pito largo de este grupo es: @${from.split("-")[0]}`, 
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				break*/
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recibido, chau gil👋😘 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
		...
		if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                                case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Ya estas registrado bebe😘')
					if (args.length < 1) return reply(`Incorrecto \nComando : ${prefix}daftar Nombre\nComando : ${prefix}daftar Gσηzαα ι∂к`)
            ...
            break
				...
				case 'nsfwass':
					...
					case 'nsfwsidebobs':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'La vieja del maty, tremenda puta'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
                ...
                default:
			      ...
			      reply(`so jnasja, no juego mas.`)
		          }
				
		if (budy.includes(`pito`)) {
                  reply(`alla le estan metiendo el pito`)
				  }
		...
		if (budy.includes(`ooopaaa`)) {
                  reply(`opaaaaa`)
                  }
                 
		if (budy.includes(`Fuaaa`)) {
                  reply(`el diegooo`)
                  }
